...
module INOURPC
  ...
  module Encoder
	...
	def check_type(arg)
	  ...
	  if c ==  Bignum then
		targ = arg.abs
		if (0x7fff_ffff_ffff_ffff & targ) == targ or arg == -1<<63 then
		  return T_INTEGER8
		else
		  return T_DECIMAL
		end
		...
		targ = arg.abs
		if (0x7f & targ) == targ or arg == -128 then
		  return T_INTEGER1
		elsif (0x7fff & targ) == targ or arg == -32768 then
		  return T_INTEGER2
		elsif (0x7fff_ffff & targ) == targ or arg == -2147483648 then
		  return T_INTEGER4
    elsif (0x7fff_ffff_ffff_ffff & targ) == targ or arg == -1<<63 then
		  return T_INTEGER8 # for jruby
		else
		  return T_DECIMAL # for jruby
		end
	  elsif c ==  Float then
		return T_DOUBLE
	  elsif c ==  String then
  ...
  class MessageServer
	...
	def initialize(comid)
	  @comid = comid

	  @logger = Logger.new(STDOUT)
	  @logger.level = Logger::INFO
	  @logger.datetime_format = get_logger_format(@comid)

	  @handler_table = Hash.new
	  @handler_lock = Monitor.new

	  @salt = "#{rand(1000000000)}:#{rand(1000000000)}"
	  @sid_counter = 0
	...
	def block_working_thread
	  ...
	  @logger.debug ":started working block."
	  @socket_lock.synchronize do
	  ...
	  loop {
		...
		@logger.debug ":   working-thread wakeup: #{@sending_thread}"
	  }
	  ...
	  @sending_queue << nil
	  @logger.debug ":finished working block."
	end

	def shutdown
	  @logger.debug ":shutdown message arived."
	  begin
		@socket_lock.synchronize do
		...
		@logger.debug ":waiting for termination of threads..."
		scount = 0
		loop {
		...
		if @sending_queue.size > 0 then
		  @logger.debug ": sending queue: #{@sending_queue.size} messages are remained."
		end
	  ensure
		@thread_pool.dispose
	  end
	...
	def send_message(name,args)
      ...
      if ret then
        ...
        @logger.debug ":  received result:  #{sid}"
        return ret.value
      else
	...
	def add_queue_calling_message(name,args)
	  ...
	  @logger.debug ": +Queue[#{@sending_queue.size}] : CALL : #{c.sid} -> #{name}"
	  return sid
	end
	...
	def sender_loop
	  loop {
		begin
		  ...
		  if entry then 
            @logger.debug ": -Queue[#{@sending_queue.size}] : #{entry.sid}"
			entry.exec(@socket)
			@logger.debug ": sent a message : #{entry.sid}"
		  end
		rescue => evar
		  mes = evar.message
		  @logger.warn "[senderloop] #{evar.to_s}  "
		  if mes["abort"] then
			@logger.warn ": [sendloop] disconnected by remote host."
		  elsif evar.class == IOError then
			@logger.warn evar.backtrace.join("\n")
			@logger.warn ": [sendloop] try to reset the connection."
			@socket_lock.synchronize do
			  @socket_state = :socket_closing
			end
		  else
			@logger.warn evar.backtrace.join("\n")
			@logger.warn ": [sendloop] going to recover the communication."
		  end
		  if entry then
            @receiving_table[entry.sid].push ResultErrObject.new(entry.sid,R_PROTOCOL_ERROR,"IOError",evar.message,evar.backtrace.join("\n"))
		  end
		ensure
		  @logger.debug ": [sendloop]--------------"
		end # rescue
		@socket_lock.synchronize do
	...
	def receiver_loop
	  loop {
		begin
		  t,mcode = read(@socket)
		  @logger.debug ": receiving a message : code=#{mcode}"
		  msid = nil
		  case mcode
		  when M_CALL
			t,msid = read(@socket)
			c = CallingOject.new.build_by_receiver(msid,@socket)
			@logger.debug ": received: OK : #{c.sid}"
			received(c)
		  when M_RETURN
			t,msid = read(@socket)
            @logger.debug ": sid=#{msid}"
			r = build_result_object(msid,@socket)
			@logger.debug ": received: RET: #{r.sid}"
            @receiving_table[r.sid].push r
		  else
			@logger.debug ": Unknown message code. try to reset the connection."
            @socket_state = :socket_closing
            @sending_queue.push nil # wakeup sender thread
			return
		  end # case
		  ...
		  @logger.debug "[rcvloop] #{evar.to_s}"
		  if mes["close"] || mes["reset"] then
			@logger.debug ": [rcvloop] disconnected by remote host."
			break
		  elsif evar.kind_of?(IOError) then
			@logger.debug ": [rcvloop] try to reset the connection."
			@socket_lock.synchronize do
			  @socket_state = :socket_closing
			end
			break
	...
	def received(calling_obj)
	  ...
	  @thread_pool.invoke lambda {
		@logger.debug ": methodInvodation: Start: #{calling_obj.name} : #{calling_obj.sid}"
		begin
		  ret = handler.call(*calling_obj.args)
          add_queue_result(calling_obj.sid,ret)
		rescue => evar
		  @logger.warn ": exception was occured: #{evar.message}"
          add_queue_error(calling_obj.sid,R_APP_ERROR,evar.class.to_s,
                          evar.message,evar.backtrace.join("\n"))
		ensure
		  @logger.debug ": methodInvodation: End  : #{calling_obj.name} : #{calling_obj.sid}"
		end # begin
	  } # lambda
	end
  ...
  class RPCMultiServer
	...
	def initialize(port = 0)
	  ...
	  @port_number = @server_socket.addr[1]

	  @logger = Logger.new(STDOUT)
	  @logger.level = Logger::INFO
	  @logger.datetime_format = get_logger_format("SV")
	...
	def start
	  if @server_socket && @server_socket.closed?
		@server_socket = TCPServer.open(@port_number)
	  end
	  begin
		accepter_loop
	  ensure
		...
		@logger.debug "RPCMultiServer: finished."
	  end
	end

    private
	def accepter_loop
	  @logger.debug "RPCMultiServer: start loop"
	  counter = 0
	  loop {
		@logger.debug "RPCMultiServer: waiting for client's connection..."
		begin
		  ...
		  if e.message == "stream closed"
			@logger.info("RPCMultiServer: stream closed")
		  else
			@logger.warn(e)
		  end
		end
		if @shutdown_flag
		  @logger.debug "RPCMultiServer: accepter loop finished"
		  @shutdown_flag = false
		  break
		end
	  }
	...
	def start_a_server(socket,counter)
	  begin
		...
		@logger.debug "RPCMultiServer#{counter}: connection established."
		@server_list.synchronize do
		...
		@logger.warn "RPCMultiServer#{counter}: finished."
	  end
	end
  ...
  class RPCClient
	...
	attr_accessor :reconnect_waittime

	def logger=(l)
	  @logger = l
	  @message_server.logger = l
	end
	
	def initialize(host,port)
	  @logger = Logger.new(STDOUT)
	  @logger.level = Logger::INFO
	  @logger.datetime_format = get_logger_format("CL")

	  @message_server = MessageServer.new("CL")
	  @message_server.logger = @logger
	...
	def set_close_handler(h)
	  @message_server.set_close_handler(h)
	end
	...
	def start
	  ...
	  @socket_thread = Thread.start do
		begin
		  ...
		  @logger.debug "RPCClient: connection-thread finished."
		  @socket_thread = nil
		end
	  end
	  loop {
	...
	def shutdown
	  @logger.debug "RPCClient: shutdown broadcast arrived."
	  @shutdown_flag = true
	  @message_server.shutdown
	  @logger.debug "RPCClient: shutdown completed."
	end
	
    private
	def connection_loop
	  @logger.debug "RPCClient: start loop"
	  loop {
		break if @shutdown_flag
		@logger.debug "RPCClient: connecting..."
        soc = TCPSocket.open(@host,@port)
		begin
		  @message_server.set_socket( soc )
		  @logger.debug "RPCClient: connection established."
		  @message_server.block_working_thread
		  @logger.debug "RPCClient: escaped working block."
		rescue => e
		  @logger.warn e.message
		  @logger.warn e.backtrace.join("\n")
		ensure
		  if soc && !soc.closed?
			soc.close
			@logger.debug "RPCClient: socket closed"
		  end
		  @logger.debug "RPCClient: disconnected."
		end
		break if !@reconnect
		sleep(@reconnect_waittime)
	  }
	end
  ...
  class ThreadPool 
	...
	def initialize(num)
	  ...
	  @worker_threads = []
	  num.times {
		@worker_threads << Thread.start(@job_queue) { |queue|
		  loop {
			job = queue.shift
			break unless job
			job.call
